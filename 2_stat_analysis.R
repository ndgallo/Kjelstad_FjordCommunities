## FjordCommunities
##
## Data analysis (updated 23 March 2025)
##
...
tiff(filename="_figures/Fig6_new.tiff",width=4000,height=4000,
...
#catch_minus_periphylla <- CPUE_catchweight_clean[, -c(29)] #Periphylla
#sum across all columns
#catch_minus_periphylla_calc <- colSums(catch_minus_periphylla)
...
ggsave(map_cluster, filename="map_cluster.tiff", width=5, height=8, units = "in", dpi = 600)
...
ggsave(nmds_plot, filename="nmds_plot.tiff", width=6.5, height=6, units = "in", dpi = 600)
...
nmds_plot_env <- ggplot() +
  ...
  ggrepel::geom_text_repel(
    ...
    label = env.scores.fac$factors.env, colour = "grey20", size = 4.5, force = 3,
  ) +
  theme_bw() +
  theme(axis.title.x = element_text(size = 13),
        axis.title.y = element_text(size = 13),
...
ggsave(nmds_plot_env, filename="nmds_plot_env.tiff", width=6.5, height=6, units = "in", dpi = 600)